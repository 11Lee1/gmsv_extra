	...
	namespace Lua
	{
		...
		public:
			...
			*/
			...
			int GetGlobalobject(char const* name)
			{
				this->PushSpecial(SPECIAL_GLOB);
				this->GetField(-1, name);
				int reference = this->ReferenceCreate();
				this->Pop();
				return reference;
			}
		...
		{
		public:
			void Push() { lua->ReferencePush(m_iref); }
			void Pop(int iAmt = 1) { lua->Pop(iAmt); }
			void PushCFunction(CFunc val) { lua->PushCFunction(val); }
			void SetField(int iStackPos, const char* strName) { lua->SetField(iStackPos, strName); }
		protected:
			BYTE PAD[0xC];
			int m_iref;
			ILuaBase* lua;
