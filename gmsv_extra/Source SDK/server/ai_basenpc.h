...
private:
	...
	virtual int			StartTask(Task_t *pTask) { StartTask((const Task_t *)pTask); return 0; } // to ensure correct signature in derived classes
	virtual int			RunTask(Task_t *pTask) { RunTask((const Task_t *)pTask); return 0; } // to ensure correct signature in derived classes
...
public:
	...
	void				DelayMoveStart(float delay) { m_flMoveWaitFinished = g_pInterfaces->Globals()->curtime + delay; }
...
public:
	...
	void				AddSceneLock(float flDuration = 0.2f) { m_flSceneTime = MAX(g_pInterfaces->Globals()->curtime + flDuration, m_flSceneTime); };
	void				ClearSceneLock(float flDuration = 0.2f) { m_flSceneTime = g_pInterfaces->Globals()->curtime + flDuration; };
	bool				IsInLockedScene(void) { return m_flSceneTime > g_pInterfaces->Globals()->curtime; };
	float				m_flSceneTime;
...
public:
	...
	virtual void NetworkStateChanged_m_lifeState();
	virtual void NetworkStateChanged_m_lifeState(void *pVar);
...
public:
	...
	void				StartPingEffect(void) { m_flTimePingEffect = g_pInterfaces->Globals()->curtime + 2.0f; DispatchUpdateTransmitState(); }
};
