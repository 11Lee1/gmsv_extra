...
public:
	...
	bool 			IsLocked() const { return (m_flNextUseTime > g_pInterfaces->Globals()->curtime); }
	void			Lock(float duration) { m_flNextUseTime = g_pInterfaces->Globals()->curtime + duration; }
	void			Unlock() { m_flNextUseTime = g_pInterfaces->Globals()->curtime; }
