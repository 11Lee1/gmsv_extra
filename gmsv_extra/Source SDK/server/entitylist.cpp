...
public:
	...
	int ListCopy(CBaseEntity *pList[], int listMax)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (m_simThinkList[i].nextThinkTick <= g_pInterfaces->Globals()->tickcount)
			{
...
public:
	...
	void FrameUpdatePostEntityThink()
	{
		...
		if (m_bRespawnAllEntities)
		{
			...
			g_pInterfaces->EngineServer()->AllowImmediateEdictReuse();

			// Reset node counter used during load
			//CNodeEnt::m_nNodeCount = 0;

			CRespawnEntitiesFilter filter;
			MapEntity_ParseAllEntities(g_pInterfaces->EngineServer()->GetMapEntitiesString(), &filter, true);

			// Allocate a CBasePlayer for pev, and call spawn
			if (nPlayerIndex >= 0)
			{
				edict_t *pEdict = g_pInterfaces->EngineServer()->PEntityOfEntIndex(nPlayerIndex);
...
public:
	...
	void ReportEntityList()
	{
		...
		{
			printf("Total %d entities (%d empty, %d edicts)\n", m_sortedList.Count(), m_emptyCount, edicts);
		}
	}
private:
