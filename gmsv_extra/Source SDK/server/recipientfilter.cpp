...
void CRecipientFilter::AddAllPlayers(void)
{
	...
	int i;
	for (i = 1; i <= g_pInterfaces->Globals()->maxClients; i++)
	{
...
void CRecipientFilter::RemoveRecipientsNotOnTeam(CTeam *team)
{
	...
	for (i = 1; i <= g_pInterfaces->Globals()->maxClients; i++)
	{
		...
		if (player->m_iTeamNum != team->m_iTeamNum)
		{
			RemoveRecipient(player);
		}
	}
...
{
	if (g_pInterfaces->Globals()->maxClients == 1)
	{
		AddAllPlayers();
	}
	else
	{
		...
		g_pInterfaces->EngineServer()->Message_DetermineMulticastRecipients(false, origin, playerbits);
...
void CRecipientFilter::AddRecipientsByPAS(const Vector& origin)
{
	...
	{
		CBitVec< ABSOLUTE_PLAYER_LIMIT > playerbits;
		g_pInterfaces->EngineServer()->Message_DetermineMulticastRecipients(true, origin, playerbits);
		AddPlayersFromBitMask(playerbits);
	}
}
...
void CPASAttenuationFilter::Filter(const Vector& origin, float attenuation /*= ATTN_NORM*/)
{
	...
	for (int i = c - 1; i >= 0; i--)
	{
		...
		CBaseEntity *ent = (CBaseEntity*)g_pInterfaces->EngineServer()->PEntityOfEntIndex(index)->GetUnknown();
		if (!ent || !ent->IsPlayer())
		{
			continue;
		}
