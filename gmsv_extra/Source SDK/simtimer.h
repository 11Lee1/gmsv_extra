...
public:
	...
	bool Expired() const
	{
		return (g_pInterfaces->Globals()->curtime - m_next > -ST_EPS);
	}
	...
	{
		m_next = g_pInterfaces->Globals()->curtime + interval;
	}

	void Set(float minInterval, float maxInterval)
	{
		if (maxInterval > 0.0)
			m_next = g_pInterfaces->Globals()->curtime + g_pInterfaces->Random()->RandomFloat(minInterval, maxInterval);
		else
			m_next = g_pInterfaces->Globals()->curtime + minInterval;
	}

	float GetRemaining() const
	{
		float result = m_next - g_pInterfaces->Globals()->curtime;
		if (result < 0)
			return 0;
		return result;
	}
...
public:
	...
	{
		m_interval = interval;
		m_next = (startExpired) ? -1.0 : g_pInterfaces->Globals()->curtime + m_interval;
	}
	...
	void Reset(float interval = -1.0)
	{
		if (interval == -1.0)
		{
			m_next = g_pInterfaces->Globals()->curtime + m_interval;
		}
		else
		{
			m_next = g_pInterfaces->Globals()->curtime + interval;
...
public:
	...
	void Set(float minInterval, float maxInterval = 0.0, bool startExpired = true)
	{
		...
		{
			if (m_maxInterval == 0)
				m_next = g_pInterfaces->Globals()->curtime + m_minInterval;
			else
				m_next = g_pInterfaces->Globals()->curtime + g_pInterfaces->Random()->RandomFloat(m_minInterval, m_maxInterval);
		}
	}
...
public:
	...
	{
		m_fIsRunning = true;
		m_next = g_pInterfaces->Globals()->curtime + intervalOverride;
	}
...
public:
	...
	void Start(float minOverride, float maxOverride = 0.0)
	{
		m_fIsRunning = true;
		if (maxOverride == 0)
			m_next = g_pInterfaces->Globals()->curtime + minOverride;
		else
			m_next = g_pInterfaces->Globals()->curtime + g_pInterfaces->Random()->RandomFloat(minOverride, maxOverride);
	}
...
public:
	...
	bool EnterThink()
	{
		if (m_lastTime == g_pInterfaces->Globals()->curtime)
			return false;
		m_lastTime = g_pInterfaces->Globals()->curtime;
		return true;
	}

	bool DidThink() const
	{
		return (g_pInterfaces->Globals()->curtime == m_lastTime);
