...
#else
	if (g_pInterfaces->StaticPropMgr()->IsStaticProp(pHandleEntity))
		return NULL;
	...
	const IHandleEntity *ignore, int collisionGroup, trace_t *ptr)
{
	...
	g_pInterfaces->EngineTrace()->TraceRay(ray, mask, &traceFilter, ptr);
}
	...
	ITraceFilter *pFilter, trace_t *ptr)
{
	...
	g_pInterfaces->EngineTrace()->TraceRay(ray, mask, pFilter, ptr);
}
...
{
	return g_pInterfaces->EngineTrace()->GetPointContents(vec);
}
